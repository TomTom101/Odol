...
Calibration ADJDS311::calibrate(Calibration cal){
	delay(2); //wait for LED to come full brintness if used
	
	Calibration _cal = Calibration();
	_cal.colorHalf = calibrateColor(cal.colorHalf);  // This calibrates R, G, and B int registers
	_cal.clearHalf = calibrateClear(cal.clearHalf);  // This calibrates the C int registers
	_cal.capacitor = calibrateCapacitors(cal.capacitor);  // This calibrates the RGB, and C cap registers
	return _cal;
}
...
of the ADJD-S311.
*/
int ADJDS311::calibrateClear(int _half){
	int gainFound = 0;
	int upperBox=4096;
	int lowerBox = 0;
	int half;
	if(_half > 0) {
	  writeInt(INT_CLEAR_LO, _half);
	  return _half;
    }
	while (!gainFound){
...
/* calibrateColor() - This function calibrates the RG and B 
integration registers.
*/
int ADJDS311::calibrateColor(int _half){
  int gainFound = 0;
  int upperBox=4096;
  int lowerBox = 0;
  int half;
  if(_half > 0) {
	writeInt(INT_RED_LO, _half);
    writeInt(INT_GREEN_LO, _half);
    writeInt(INT_BLUE_LO, _half);
	return _half;
  }
...
capacitor registers.
*/
calRGBC ADJDS311::calibrateCapacitors(calRGBC cal){
  ...
  if(cal.red) {
    writeRegister(cal.red, CAP_RED);
    writeRegister(cal.green, CAP_GREEN);
    writeRegister(cal.blue, CAP_BLUE);
	return cal;
  }
  ...
  calRGBC rCal = calRGBC();
  rCal.red = calibrationRed;
  rCal.green = calibrationGreen;
  rCal.blue = calibrationBlue;
